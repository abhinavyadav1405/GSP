...
// 3. Clean injection into Settings Page
...
if (c.includes(target)) {
  // If already contains Manage All 3 Admins, replace from settings target to the end of that block or just fresh replace if not cleanly formatted
  // Let's do a safe replacement
  const parts = c.split(target);
  if (parts.length > 1) {
    // Find where the old settings block ends or just replace the opening
    c = parts[0] + cleanSettingsBlock + parts[1].substring(parts[1].indexOf('</FadeIn>'));
    fs.writeFileSync(f, c);
    console.log('✅ Clean Admin Settings successfully applied!');
  }
} else {
  console.log('⚠️ Settings target not found.');
}
