...
// 3. Target "Change Super Admin Password" to place the multi-admin box right above it in settings
...
if (c.includes(target)) {
  // Remove any previous duplicate insertion if present
  if (c.includes('Manage All 3 Admins')) {
    // Clean up old matches to prevent duplicate declarations in source
    const parts = c.split('Manage All 3 Admins');
    if (parts.length > 2) {
      // Re-read clean file or let's just do a clean replacement
    }
  }
  c = c.replace(target, multiAdminCard);
  fs.writeFileSync(f, c);
  console.log('✅ Definitive 3-Admin section injected successfully!');
} else {
