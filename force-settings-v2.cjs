...
// 3. Find where settings page starts and inject the 3-admin panel at the very top of settings view
const target = '{page === "settings" && (';
const multiAdminSettingsBox = `
...
if (c.includes(target) && !c.includes('Manage All 3 Admins')) {
  // Insert right after page === "settings" && (
  c = c.replace(target, target + multiAdminSettingsBox);
  fs.writeFileSync(f, c);
  console.log('✅ 3-Admin section injected successfully into Settings view!');
} else {
  console.log('⚠️ Target already contains it or could not find settings view block.');
}
