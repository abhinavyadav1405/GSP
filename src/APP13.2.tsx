    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000,
          system:`You are a helpful AI assistant for "${villageName}" — a Gram Panchayat Portal in India. Sarpanch: ${sarpanchName}. Villagers report infrastructure problems, track resolution, see notices, give feedback. Answer concisely in English under 3 sentences.`,
          messages:[...msgs.map(m=>({role:m.role==="user"?"user":"assistant",content:m.text})),{role:"user",content:text}],
        }),
      });
  ...
  return (
    <div style={{ width:"100%" }}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <span style={{ flex:1,textAlign:"left" }}>Ask AI about this portal</span>
      ...
      {open && (
        <div className="glass" style={{ marginTop:10,padding:0,overflow:"hidden",borderRadius:20 }}>
          <div style={{ padding:"12px 18px",borderBottom:"1px solid rgba(124,92,252,0.10)",display:"flex",alignItems:"center",gap:10 }}>
            ...
            <div><div style={{ fontSize:13,fontWeight:700,color:"var(--text-main)" }}>AI Assistant</div><div style={{ fontSize:11,color:"var(--ct4)" }}>Ask anything about {villageName}</div></div>
          ...
          <div ref={chatRef} style={{ height:220,overflowY:"auto",padding:"12px 14px 8px",display:"flex",flexDirection:"column",gap:10 }}>
            {msgs.length === 0 && (
              <div style={{ textAlign:"center",padding:"12px 0" }}>
                ...
                <div style={{ fontSize:13,color:"var(--ct4)",marginBottom:12 }}>Ask me anything about this portal</div>
          ...
          <div style={{ padding:"10px 12px",borderTop:"1px solid rgba(124,92,252,0.08)",display:"flex",gap:8,alignItems:"center" }}>
            <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key==="Enter" && ask(query)} placeholder="Type your question…" style={{ flex:1,padding:"8px 12px",borderRadius:11,fontSize:13 }}/>
