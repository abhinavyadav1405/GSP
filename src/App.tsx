...
function PhoneLogin({ onClose }) {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      setStep("otp");
    } catch(e) { setError("OTP nahi aaya: " + e.message); window.recaptchaVerifier = null; }
    setLoading(false);
  };
  ...
  const verifyOTP = async () => {
    ...
    } catch(e) { setError("Galat OTP!"); }
    setLoading(false);
  };

  const saveName = async () => {
    if (!name.trim()) { setError("Naam zaroori hai"); return; }
    setLoading(true);
    try {
  ...
  return (
    React.createElement("div", { style: { position:"fixed", inset:0, zIndex:9999, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 } },
      React.createElement("div", { id:"recaptcha-container" }),
      React.createElement("div", { className:"glass", style:{ borderRadius:22, padding:"32px 24px", width:"100%", maxWidth:380, textAlign:"center" } },
        step === "phone" && React.createElement(React.Fragment, null,
          React.createElement("div", { style:{ fontSize:40, marginBottom:12 } }, "📱"),
          React.createElement("h2", { style:{ color:"var(--text-main)", fontSize:20, marginBottom:6 } }, "Login Karein"),
          React.createElement("p", { style:{ color:"var(--ct4)", fontSize:13, marginBottom:24 } }, "Mobile number se OTP aayega"),
          React.createElement("input", { value:phone, onChange:e=>setPhone(e.target.value.replace(/\D/,"").slice(0,10)), placeholder:"10 digit mobile number", type:"tel", style:{ marginBottom:10 } }),
          error && React.createElement("div", { style:{ color:"#f87171", fontSize:12, marginBottom:8 } }, error),
          React.createElement("button", { className:"btn-white", onClick:sendOTP, disabled:loading, style:{ borderRadius:12, padding:"12px 0", width:"100%", fontSize:14 } }, loading ? "Bhej raha hai..." : "OTP Bhejo →"),
          React.createElement("button", { onClick:onClose, style:{ background:"none", border:"none", color:"var(--ct4)", fontSize:12, cursor:"pointer", marginTop:14 } }, "Baad mein")
        ),
        step === "otp" && React.createElement(React.Fragment, null,
          React.createElement("div", { style:{ fontSize:40, marginBottom:12 } }, "🔐"),
          React.createElement("h2", { style:{ color:"var(--text-main)", fontSize:20, marginBottom:6 } }, "OTP Daalo"),
          React.createElement("p", { style:{ color:"var(--ct4)", fontSize:13, marginBottom:24 } }, "+91 " + phone + " pe bheja gaya"),
          React.createElement("input", { value:otp, onChange:e=>setOtp(e.target.value.replace(/\D/,"").slice(0,6)), placeholder:"6 digit OTP", type:"tel", style:{ textAlign:"center", fontSize:20, letterSpacing:8, marginBottom:10 } }),
          error && React.createElement("div", { style:{ color:"#f87171", fontSize:12, marginBottom:8 } }, error),
          React.createElement("button", { className:"btn-white", onClick:verifyOTP, disabled:loading, style:{ borderRadius:12, padding:"12px 0", width:"100%", fontSize:14 } }, loading ? "Verify ho raha..." : "Verify ✓"),
          React.createElement("button", { onClick:()=>{setStep("phone");setOtp("");setError("");}, style:{ background:"none", border:"none", color:"var(--ct4)", fontSize:12, cursor:"pointer", marginTop:14 } }, "← Wapis")
        ),
        step === "name" && React.createElement(React.Fragment, null,
          React.createElement("div", { style:{ fontSize:40, marginBottom:12 } }, "👤"),
          React.createElement("h2", { style:{ color:"var(--text-main)", fontSize:20, marginBottom:6 } }, "Aapka Naam"),
          React.createElement("input", { value:name, onChange:e=>setName(e.target.value), placeholder:"Pura naam", style:{ marginBottom:10 } }),
          error && React.createElement("div", { style:{ color:"#f87171", fontSize:12, marginBottom:8 } }, error),
          React.createElement("button", { className:"btn-white", onClick:saveName, disabled:loading, style:{ borderRadius:12, padding:"12px 0", width:"100%", fontSize:14 } }, loading ? "Save ho raha..." : "Shuru Karein →")
        )
      )
    )
  );
        ...
        {page === "home" && (
          <div>
            <div style={{ padding: "60px 0 48px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: "clamp(22px,4vw,40px)" }}>
