...
function UserSettingsPage({ user, onSave, onBack }: { user: AppUser; onSave: (u: AppUser) => Promise<void>; onBack: () => void }) {
  ...
  const [theme,setTheme]=useState(localStorage.getItem("gsp-user-theme") || "system");
  ...
  const handleAvatar=async(file: File)=>{
    ...
    try{
      ...
      console.error(e);
      alert("Photo upload nahi ho paya.");
    }finally{
  ...
  const save=async()=>{
    const next={...user,name:name.trim(),mobile:mobile.trim(),ward,avatar};
    await onSave(next);
    localStorage.setItem("gsp-user-theme",theme);
    onBack();
  };
    ...
    <div className="glass" style={{borderRadius:22,padding:24}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginBottom:22}}>
        <div
          ...
          style={{
            ...
            boxShadow:"0 8px 30px rgba(124,92,252,0.30)"
          }}
        >
          {avatar
            ? <img src={avatar} alt="Profile" style={{width:"100%",height:"100%",objectFit:"cover"}} />
            : <span style={{fontSize:42,fontWeight:800,color:"#fff"}}>{name.slice(0,1).toUpperCase() || "U"}</span>
          }
        </div>
        ...
        {avatar && !avatarLoading && (
          <button
            ...
            style={{marginTop:6,border:"none",background:"none",color:"#f87171",fontSize:11,cursor:"pointer"}}
          >
            Remove photo
          </button>
        )}

        <div style={{fontSize:11,color:"var(--ct3)",marginTop:6}}>
          JPG, PNG or WebP · photo will be compressed automatically
        </div>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button className="btn-ghost" onClick={onBack} style={{flex:1,borderRadius:10}}>Cancel</button>
          <button className="btn-white" onClick={save} disabled={avatarLoading} style={{flex:1,borderRadius:10}}>
            Save Settings
          </button>
        </div>
      </div>
